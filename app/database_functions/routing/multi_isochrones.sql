...
BEGIN
    SELECT
        variable_simple::NUMERIC INTO max_length_links
    FROM
        variable_container
    WHERE
        identifier = 'max_length_links';
    SELECT
        select_from_variable_container ('excluded_class_id_walking')::text[],
        select_from_variable_container ('categories_no_foot')::text INTO excluded_class_id,
        categories_no_foot;
    SELECT
        array_agg(series) INTO objectids_array
    FROM (
        SELECT
            generate_series(1, array_length(points_array, 1)) AS series) x;
    ...
    SELECT
        ...
        objectid
    FROM
        pgrouting_edges_multi (userid_input,
            minutes,
            points_array,
            speed_input::NUMERIC,
            objectids_array,
    ...
    ALTER TABLE temp_catchment_vertices
    ...
    SELECT
        *
    FROM
        temp_extrapolated_reached_vertices;
    ...
    FROM
        temp_catchment_vertices LOOP
            ...
            SELECT
                start_vertex, node, edge, cnt, COST, geom, objectid
            FROM
                temp_catchment_vertices
            WHERE
                objectid = i;
            ...
            SELECT
                *
            FROM
                extrapolate_reached_vertices (minutes * 60,
                    max_length_links,
                    (speed_input / 3.6),
                    excluded_class_id,
                    categories_no_foot);
            LOOP
                ...
                IF (
                    ...
                    SELECT
 					...
 					FROM temp_step_vertices',
                                alphashape_parameter_input),
                            4326),
                        (upper_limit / 60)::integer;
                END IF;
...
/*SELECT * 
FROM multi_isochrones(100,15,5,5,0.00003,1,ARRAY[[11.5669,48.1546],[11.5788,48.1545]])
 */
