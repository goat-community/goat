...
std::vector<std::array<double, 2>> line_substring(const double &start_perc, const double &end_perc, const std::vector<std::array<double, 2>> &geometry, const double &total_length)
{
  ...
  for (size_t i = 0; i < geometry.size(); ++i)
  {
    ...
    double dx = geometry[i][0] - geometry[i + 1][0];
    double dy = geometry[i][1] - geometry[i + 1][1];
    double dist = sqrt(pow(dx, 2) + pow(dy, 2));
    // Get percentage at node.
    double current_perc = total_length / current_dist;
    }
  return {
      {2.33, 3.333}, {1.22, 3.555}};
}
