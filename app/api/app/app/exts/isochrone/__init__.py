...
def calculate(
    network: array, start_vertices: array, distance_limits: array, only_minimum_cover=True
) -> array:
    """
    ...
    Parameters
    ----------
    network : dtype :numpy.array{formats: [i8,i8,i8,D,D], names: [id, source, target, cost, reverse_cost]})
    ...
    network = genfromtxt(
        "./app/exts/isochrone/cpp/data/small_network.csv", delimiter=",", dtype=dt, skip_header=1
    )
    # c++ wrapper function
    return isochrone.isochrone(network, start_vertices, distance_limits, only_minimum_cover)
