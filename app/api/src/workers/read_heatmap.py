...
@celery_app.task(time_limit=settings.CELERY_TASK_TIME_LIMIT)
def read_pt_station_count_task(current_user, payload):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    result = loop.run_until_complete(read_pt_station_count_async(current_user, payload))
    return result


@celery_app.task(time_limit=settings.CELERY_TASK_TIME_LIMIT)
def read_pt_oev_gueteklassen_task(current_user, payload):
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    result = loop.run_until_complete(
        read_pt_oev_gueteklassen_async(current_user, payload)
    )
    return result
