...
class OpportunityGroup(SQLModel, table=True):
    ...
    opportunity_default_configs: Optional[List["OpportunityDefaultConfig"]] = Relationship(back_populates="opportunity_group")
    opportunity_study_area_configs: Optional[List["OpportunityStudyAreaConfig"]] = Relationship(back_populates="opportunity_group")
    opportunity_user_configs: Optional[List["OpportunityUserConfig"]] = Relationship(back_populates="opportunity_group")
...
class OpportunityDefaultConfig(OpportunityConfigBase, table=True):
    ...
    opportunity_group_id: Optional[int] = Field(sa_column=Column(Integer,  ForeignKey("basic.opportunity_group.id"), nullable=False))
    opportunity_group: "OpportunityGroup" = Relationship(back_populates="opportunity_default_configs")
...
class OpportunityStudyAreaConfig(OpportunityConfigBase, table=True):
    ...
    opportunity_group: "OpportunityGroup" = Relationship(back_populates="opportunity_study_area_configs")

UniqueConstraint(OpportunityStudyAreaConfig.__table__.c.category, OpportunityStudyAreaConfig.__table__.c.study_area_id)
...
class OpportunityUserConfig(OpportunityConfigBase, table=True):
    ...
    user_id: int = Field(sa_column=Column(Integer, ForeignKey("customer.user.id", ondelete="CASCADE")))
    data_upload_id: Optional[int] = Field(sa_column=Column(Integer, ForeignKey("customer.data_upload.id", ondelete="CASCADE")))
...
UniqueConstraint(OpportunityUserConfig.__table__.c.category, OpportunityUserConfig.__table__.c.study_area_id, OpportunityUserConfig.__table__.c.user_id)