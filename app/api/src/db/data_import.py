...
class DataImport():
    ...
    def __init__(self):
        self.required_table_to_restore = [ 
            ...
            GridVisualizationDB,
            GridCalculationDB,
            GridVisualizationParameterDB,
            StudyAreaGridVisualizationDB,
            NodeDB,
            EdgeDB,
            AoiDB,
            PoiDB,
        ...
        self.optional_table_to_restore = [
            ReachedEdgeHeatmapDB,
            ReachedEdgeHeatmapGridCalculationDB,
            ReachedPoiHeatmapDB,
            ReachedPoiHeatmapAccessibilityDB
        ]
    ...
    async def import_table(self, table, db: AsyncSession, db_staging: AsyncSession) -> dict:     
        ...
        print('INFO: There is no default data for table [bold magenta]%s[/bold magenta]. Data will be uploaded.' % table.__tablename__)
        ...
        for row in rows.scalars():
            ...
            if row_cnt % 10000 == 0:      
                ...
                await db.commit()
                print('    %s rows added to table [bold magenta]%s[/bold magenta].' % (row_cnt, table.__tablename__))
                bulk_rows = []
