CREATE OR REPLACE FUNCTION basic.get_poi_more_entrance_sum(user_id_input integer, modus text, extent geometry, pixel_resolution integer, 
scenario_id_input integer DEFAULT 0, active_upload_ids integer[] DEFAULT '{}'::integer[])
 RETURNS TABLE(pixel int[], category text, name TEXT, cnt integer)
 LANGUAGE plpgsql
AS $function$
DECLARE 
...
BEGIN 		
	...
	(
   	...
   	GROUP BY p.pixel, p.category, p.name; 
...
SELECT * 
FROM basic.get_poi_more_entrance_sum(15, 'default'::text, ST_SETSRID(ST_BUFFER(ST_MAKEPOINT(11.57616,48.13168)::geography, 200000)::geometry, 4326), 10)
 */