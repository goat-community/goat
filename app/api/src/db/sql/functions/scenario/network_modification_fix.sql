...
SELECT *
FROM basic.network_modification(474)

DELETE FROM customer.way_modified 
WHERE scenario_id = 474

INSERT INTO customer.way_modified (geom, surface, wheelchair, way_type, edit_type, scenario_id, class_id, way_id )
SELECT geom, surface, wheelchair, way_type, edit_type, scenario_id, class_id, way_id 
FROM "QueryLayer" ql 
...
DELETE FROM basic.edge WHERE scenario_id = 474
...
BEGIN
	...
	WHERE e.scenario_id = 474;
	DELETE FROM basic.node n 
	WHERE n.scenario_id = 474; 
	...
	WITH to_add AS
	(
		SELECT 474 AS scenario_id, x.closest_point AS geom 
	...
	LOOP 
		...
		ST_LENGTH(rec.geom::geography), ST_LENGTH(ST_TRANSFORM(rec.geom, 3857)), 474, rec.original_id
		;		
	END LOOP;
	/*END $$;*/
...
CREATE TABLE test_nodes AS 
SELECT *
FROM basic.node e
WHERE ST_Intersects(e.geom, ST_BUFFER(ST_SETSRID(ST_MAKEPOINT(11.5539456, 48.2658729), 4326), 0.01))
...
48.2675081,11.5526613


{'x': 11.5526613, 'y': 48.2675081, 'max_cutoff': 600, 'speed': 1.3888888888888888, 'modus': 'scenario', 'scenario_id': 474, 'routing_profile': 'walking_standard', 'table_prefix': None}


{'x': 11.5526613, 'y': 48.2675081, 'max_cutoff': 600, 'speed': 1.3888888888888888, 'modus': 'default', 'scenario_id': 474, 'routing_profile': 'walking_standard', 'table_prefix': None}

DROP TABLE network_default; 
CREATE TABLE network_default AS 
SELECT *, ST_TRANSFORM(ST_SETSRID(ST_GeomFromGeoJSON('{"type":"LineString","coordinates":'||coordinates_3857||'}'), 3857), 4326) AS geom   
FROM basic.fetch_network_routing(ARRAY[11.5526613],ARRAY[48.2675081], 1200., 1.33, 'default', 474, 'walking_standard')


DROP TABLE network_scenario;
CREATE TABLE network_scenario AS 
SELECT *, ST_TRANSFORM(ST_SETSRID(ST_GeomFromGeoJSON('{"type":"LineString","coordinates":'||coordinates_3857||'}'), 3857), 4326) AS geom  
FROM basic.fetch_network_routing(ARRAY[11.5526613],ARRAY[48.2675081], 1200., 1.33, 'scenario', 474, 'walking_standard')
