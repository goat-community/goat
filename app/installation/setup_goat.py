...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb  7 19:20:40 2019
...
os.system('''for file in ../data_preparation/SQL/*; do PGPASSFILE=~/.pgpass psql -d %s -U %s -h %s -f $file 2>/dev/null; done 
          :''' % (db_name,user,host))

os.system('''for file in ../database_function/*; do PGPASSFILE=~/.pgpass psql -d %s -U %s -h %s -f $file 2>/dev/null; done 
          :''' % (db_name,user,host))