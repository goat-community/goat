...
def recompute_heatmap(scenario_id):
    ...
    for g in gridids:
        db.perform("""SELECT compute_area_isochrone(%(grid_id)s,%(scenario_id)s)""", {"grid_id": g[0],"scenario_id": scenario_id})
    
    buffer_geom = db.select("""SELECT ST_BUFFER(ST_UNION(geom),0.0014) 
