...
GetBaseLayers().forEach(function(layer, index, array) {
  if (layer.get("name") == "StudyArea") {
    ...
    layer.getSource().on("change", function(e) {
      ...
      var geographicCenter = transform(
        [center[0], center[1]],
        "EPSG:3857",
        "EPSG:4326"
      );
      ...
      var mask = new Mask({
        feature: f,
        inner: false,
        fill: new Fill({ color: [169, 169, 169, 0.8] })
      });
      for (i of array) {
        i.addFilter(mask);
      }
...
$("body").on(
  "change",
  ".thematic_data_weight, .thematic_item_check",
  function() {
    var selectedWalkabilityType = $("#accessibility_basemap_select").val();
    if (
      ["no_basemap", "heatmap_population", "heatmap_area_isochrone"].indexOf(
        selectedWalkabilityType
      ) == -1
    ) {
  ...
  if (style != "no_basemap") {
    ...
    map.getLayers().forEach(function(layer) {
      if (
        ...
        (layer.get("name") === "layer_accessibility")
      ) {
