...
def import_raw_layer(filepath,db_conn,db_name,user,host):
    ...
    if data_type == 'shp':
        ...
        db_conn.perform(filename)
    else:
        return 

import_raw_layer('/opt/data/buildings.shp', db, 'goattemp', 'goat', 'localhost')
...
def produce_population_points(db_conn,source_population):
    ...
    for f in raw_files:
        import_raw_layer('/opt/data/'+f, db, 'goattemp', 'goat', 'localhost')

    if db.select("SELECT * FROM to_regclass('public.buildings')")[0][0] != None:
    ...
    if (source_population == 'extrapolation'):
        db_conn.perform(open('../data_preparation/SQL/'+script_buildings, "r").read())
        ...
        db_conn.execute_script_psql('../data_preparation/SQL/'+script_buildings)
        db_conn.execute_script_psql('../data_preparation/SQL/population_disagregation.sql')
    elif(source_population == 'distribution'):
        db_conn.execute_script_psql('../data_preparation/SQL/population_distribution.sql')

    db_conn.perform('../data_preparation/SQL/create_population_userinput.sql')

#produce_population_points('extrapolation', db)
