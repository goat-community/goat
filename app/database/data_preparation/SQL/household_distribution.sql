...
UPDATE points_res
SET gid = census_household.gid::integer
FROM census_household
...
FROM points_res p, census_household c
WHERE p.gid=c.gid
GROUP BY p.gid;

ALTER TABLE gridandpoints ADD COLUMN hh integer;
ALTER TABLE gridandpoints ADD COLUMN hh_per_point numeric;

--calculation of residents per points

UPDATE gridandpoints
SET hh = census_household.hh::integer
FROM census_household
WHERE gridandpoints.gid = census_household.gid;

UPDATE gridandpoints SET hh_per_point = CEIL(hh/npoints);

--preparation of population layer

CREATE TABLE households as
SELECT points_res.*, gridandpoints.hh_per_point as households
...
ALTER TABLE households ADD COLUMN id serial;
