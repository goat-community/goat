...
CREATE OR REPLACE FUNCTION public.pgrouting_edges_preparation_pois(cutoffs double precision[], startpoints double precision[], speed numeric, modus_input integer, routing_profile text, userid_input integer DEFAULT 0, scenario_id_input integer DEFAULT 0, heatmap_bulk_calculation boolean DEFAULT false, gids_scenario integer[] DEFAULT ARRAY[]::integer[])
 RETURNS SETOF void
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN 
	...
	ELSE 
		...
		FROM (SELECT unnest_2d_1d(startpoints) point, unnest(gids_scenario) gid) AS points; 
	END IF; 
