...
CREATE OR REPLACE FUNCTION public.heatmap_competition(amenities_json jsonb, modus_input text, scenario_id_input integer)
 RETURNS TABLE(grid_id integer, percentile_accessibility integer, accessibility_index bigint, percentile_ai_pop integer, ai_pop float, modus text, geom geometry)
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
	...
	IF modus_input IN ('default','comparison','comparison') THEN
		DROP TABLE IF EXISTS grids_default; 
		CREATE TEMP TABLE grids_default AS 
		SELECT g.grid_id, COALESCE(h.percentile_accessibility,0) AS percentile_accessibility, h.accessibility_index, COALESCE(h.percentile_ai_pop,0) AS percentile_ai_pop, h.ai_pop, modus_input AS modus, g.geom  
		FROM grid_heatmap g
		LEFT JOIN (
			SELECT h.grid_id, ntile(5) over (order by h.accessibility_index) AS percentile_accessibility,
			h.accessibility_index, ntile(5) over (order by h.ai_pop) AS percentile_ai_pop,
			h.ai_pop
			FROM heatmap_dynamic_population(amenities_json,'default',scenario_id_input) h
