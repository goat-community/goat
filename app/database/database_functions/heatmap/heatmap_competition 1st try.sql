DROP FUNCTION IF EXISTS heatmap_competition;
CREATE OR REPLACE FUNCTION public.heatmap_competition(amenities_json jsonb, modus_input text DEFAULT 'default', scenario_id_input integer DEFAULT 0)
 RETURNS TABLE(gid integer, accessibility_index bigint)
 LANGUAGE plpgsql
AS $function$
DECLARE
	...
	translation_sensitivities_short jsonb;
...
BEGIN
	...
	SELECT (s.sensitivities  # (v ->> 'sensitivity')::integer)::smallint as position
	INTO translation_sensitivities_short
	FROM jsonb_each('{"kindergarten":{"sensitivity":250000,"weight":1}}'::jsonb) AS u(k, v), sensitivities s;


/*Calculate AI from pois to population*/
	/*IF modus_input = 'default' THEN*/
		RETURN query 
		SELECT s.gid, sum(s.accessibility_index) AS accessibility_index 
		FROM 
		(

			SELECT x.pop_gid, u.gid, u.accessibility_index::SMALLINT AS accessibility_index, (u.accessibility_index*x.population)::SMALLINT AS accessibility_pop 
			FROM (
				SELECT r.pop_gid, r.gids, r.accessibility_indices[(s.position)::integer:(s.position)::integer][1:], p.population
				FROM reached_population_pois r, translation_sensitivities_short s, population p
				WHERE r.scenario_id = 0				
			)x, UNNEST(x.gids, x.accessibility_indices) AS u(gid, accessibility_index)
			UNION ALL 
			SELECT x.pop_gid, u.gid, max(u.accessibility_index)::SMALLINT AS accessibility_index, (max(u.accessibility_index)*x.population)::SMALLINT AS accessibility_pop
			FROM (
				SELECT pop_gid, gids, r.accessibility_indices[(s.position)::integer:(s.position)::integer][1:], p.population
				FROM reached_population_pois r,translation_sensitivities_short s, population p
				WHERE r.scenario_id = 0
			)x, UNNEST(x.gids, x.accessibility_indices) AS u(gid, accessibility_index)
			GROUP BY x.pop_gid, u.gid, x.population
		) s
		GROUP BY s.gid;
/*	ELSE
...
/*Calculate AI from grid to pois*/
/*	IF modus_input = 'default' THEN 
		RETURN query 
		SELECT s.grid_id, sum(s.accessibility_index) AS accessibility_index 
		FROM 
