DROP FUNCTION IF EXISTS heatmap_gravity_exp;
CREATE OR REPLACE FUNCTION public.heatmap_gravity_exp(amenities_json jsonb, modus_input text, scenario_id_input integer)
...
BEGIN
	...
	IF modus_input IN ('default','comparison') THEN   
		...
		LEFT JOIN (
			...
			FROM heatmap_dynamic_exp(amenities_json,'default',scenario_id_input) h
	...
	IF modus_input IN ('scenario','comparison') THEN  
		...
		LEFT JOIN (
			SELECT h.grid_id, ntile(5) over (order by h.accessibility_index) AS percentile_accessibility,
			h.accessibility_index
			FROM heatmap_dynamic_exp(amenities_json,modus_input,scenario_id_input) h
			WHERE h.accessibility_index IS NOT NULL
		) h
		ON g.grid_id = h.grid_id;	
