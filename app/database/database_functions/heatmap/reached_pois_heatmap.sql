...
BEGIN 
	...
	IF combination_scenarios = 0 THEN 
	    ...
	    SELECT p.gid, p.amenity, p.name, p.gridids, c.arr_true_cost arr_cost, p.edge, p.fraction, c.accessibility_indices::integer[], scenario_id_input scenario_id 
	...
	ELSE 
	    ...
	    (
	        ...
	        WHERE p.amenity IN (SELECT UNNEST(select_from_variable_container('pois_one_entrance') || select_from_variable_container('pois_more_entrances')))
	        AND ST_Intersects(p.geom,buffer_geom)
	        AND p.scenario_id = scenario_pois
	    )
	    SELECT p.gid, p.amenity, p.name, p.gridids, c.arr_true_cost arr_cost, p.edge, p.fraction, c.accessibility_indices::integer[], scenario_id_input scenario_id 
	    FROM p, compute_accessibility_index(fraction,start_cost,end_cost,sensitivities) c;
	END IF;
	   
   	INSERT INTO reached_pois_heatmap(gid, amenity, name, gridids, arr_cost, edge, fraction, accessibility_indices, scenario_id)
	...
	SELECT p.gid, p.amenity, p.name, e.gridids, c.arr_true_cost arr_cost, e.edge, p.fraction, c.accessibility_indices::integer[], scenario_id_input scenario_id
	...
	(
		...
		update_accessibility_array(x.a1, x.a2, arr_position) AS accessibility_indices, scenario_id_input scenario_id
	...
	(
		...
		p.accessibility_indices AS accessibility_indices, p.scenario_id 
		FROM reached_pois_heatmap r, reached_pois_partial p  
		WHERE r.gid = p.gid 
	...
	SELECT g.gid, g.gridids, g.arr_cost, append_accessibility_array(r.accessibility_indices,s.accessibility_indices) accessibility_indices, g.scenario_id 
