DROP FUNCTION IF EXISTS heatmap_dynamic_iapi;
CREATE OR REPLACE FUNCTION public.heatmap_dynamic_iapi(amenities_json jsonb, modus_input text DEFAULT 'default', scenario_id_input integer DEFAULT 0)
...
SELECT h.*, g.geom  
FROM heatmap_dynamic_iapi('{"kindergarten":{"sensitivity":250000,"weight":1}}'::jsonb,'scenario',13) h, grid_heatmap g
WHERE h.grid_id = g.grid_id; 

DROP TABLE default_;
CREATE TABLE default_ AS 
SELECT h.*, g.geom  
FROM heatmap_dynamic_iapi('{"kindergarten":{"sensitivity":250000,"weight":1}}'::jsonb,'default',13) h, grid_heatmap g
WHERE h.grid_id = g.grid_id; 
*/