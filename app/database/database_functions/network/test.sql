...
BEGIN
	...
	/*It deletes all parts that are smaller then one centimeter*/
	...
	*/
	...
	DROP TABLE IF EXISTS ways_attributes_only;
	CREATE TEMP TABLE ways_attributes_only AS 
	SELECT w.id, w.class_id, m.surface, m.foot, m.bicycle, w.geom, w.SOURCE, w.target, m.lit, m.wheelchair, NULL   
	FROM ways_modified m, ways_userinput w 
	WHERE w.scenario_id IS NULL 
	AND m.scenario_id = 2
	AND m.edit_type = 'attributes_only'
	AND w.id = m.original_id; 
