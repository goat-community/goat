...
def real_columns(properties: Optional[List[Any]]) -> List[str]:
    ...
    cols = [
        c.description for c in properties if hasattr(c, 'description') and c.type not in ["geometry", "geography"]
    ]
...
class ExtCollection(Collection):
      ...
      if ids is not None:
          if len(ids) == 1:
              wheres.append(
                  ...
                  == pg_funcs.cast(
                      pg_funcs.cast(ids[0], "text"), self.id_column_info.type # type: ignore
                  )
              )
          else:
              w = [
                  logic.V(self.id_column)
                  == logic.S(
                      pg_funcs.cast(pg_funcs.cast(i, "text"), self.id_column_info.type) # type: ignore
                  )
                  for i in ids
              ]
  ...
  def _select_no_geo(self, properties: Optional[List[str]], addid: bool = True) -> BuildPgSqlBlock:
